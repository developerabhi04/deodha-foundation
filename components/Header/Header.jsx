                ...
                <div className="container mx-auto px-4 md:px-6 xl:px-8">
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <Link href="/" className="flex items-center gap-2 md:gap-3 group relative z-10 flex-shrink-0">
                            ...
                            <div className={`relative w-10 h-10 md:w-12 md:h-12 lg:w-14 lg:h-14 rounded-lg flex items-center justify-center shadow-lg transition-all overflow-hidden ${scrolled ? 'bg-transparent' : 'bg-transparent'
                                }`}>
                                <Image
                                    src={logo}  // Place logo in /public/logo.jpeg
                        ...
                        <nav className="hidden xl:flex items-center gap-1 2xl:gap-2">
                            {navLinks.map((link) => (
                                <Link
                                    key={link.href}
                                    href={link.href}
                                    className={`px-3 2xl:px-4 py-2 text-xs 2xl:text-sm font-bold uppercase tracking-wide transition-all relative group ${isActive(link.href)
                                        ? scrolled
                                            ? 'text-brand-green'
                                            : 'text-green-500'
                                        : scrolled
                                ...
                                >
                                    {link.label}
                                    <span className={`absolute bottom-0 left-0 w-0 h-0.5 transition-all group-hover:w-full ${scrolled ? 'bg-brand-green' : 'bg-green-700'
                                        }`}></span>
                                </Link>
                            ))}
                        </nav>
                        ...
                        <div className="flex items-center gap-2 md:gap-3">
                            ...
                            <div className="hidden lg:block relative group">
                                <button className={`flex items-center gap-2 px-3 md:px-4 py-2 rounded-lg font-bold text-xs md:text-sm transition-all ${scrolled
                                ...
                                <div className="absolute right-0 top-full mt-2 w-36 bg-white rounded-lg shadow-2xl border border-gray-100 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2 text-sm font-semibold hover:bg-green-50 transition-colors ${language === 'en' ? 'text-brand-green bg-green-50' : 'text-gray-700'
                                            }`}
                                    >
                                        🇬🇧 English
                                    </button>
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2 text-sm font-semibold hover:bg-green-50 transition-colors ${language === 'hi' ? 'text-brand-green bg-green-50' : 'text-gray-700'
                                            }`}
                                    >
                                        🇮🇳 हिन्दी
                                    </button>
                            ...
                            <button
                                ...
                                className={`xl:hidden p-2 rounded-lg transition-all ${scrolled
